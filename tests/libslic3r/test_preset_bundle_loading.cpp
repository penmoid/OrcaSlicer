...
// capability's AMS-sync filament-resolution hook). No Python/plugin layer involved: the resolver is
// a plain C++ lambda installed directly, exactly as PluginHooks.cpp would install one.
namespace {

// The resolver hook is process-global state; reset it in every TEST_CASE that installs one, even
// if a REQUIRE aborts the test partway through.
struct ResolverGuard {
    ~ResolverGuard() { PresetBundle::set_ams_filament_resolver_fn(nullptr); }
};

// One AMS tray's raw config, in the shape Sidebar::build_filament_ams_list() (Plater.cpp) produces:
// every field is a single-element ConfigOptionStrings, read back via opt_string(key, 0u).
...
TEST_CASE("A resolver-returned compatible preset wins over stock AMS matching", "[Preset][AmsResolver]")
{
    ResolverGuard guard;
    PresetBundle  bundle;
...
TEST_CASE("A declined or invalid resolver answer falls through to stock AMS matching", "[Preset][AmsResolver]")
{
    ResolverGuard guard;
    PresetBundle  bundle;
    ...
    Preset &generic       = add_inmemory_preset(bundle.filaments, "Generic PLA");
...
TEST_CASE("The AMS resolver is not consulted in color-only sync mode", "[Preset][AmsResolver]")
{
    ResolverGuard guard;
    PresetBundle  bundle;
...
TEST_CASE("Resetting the AMS resolver restores stock matching", "[Preset][AmsResolver]")
{
    ResolverGuard guard;
    PresetBundle  bundle;
